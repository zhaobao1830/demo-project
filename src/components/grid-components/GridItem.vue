<template>
  <div ref="item"
       class="vue-grid-item"
       :class="{classObj, 'is-active': active}"
       :style="style"
    >
    <slot></slot>
...
<script>
  import { setTopLeft, setTopRight, setTransformRtl, setTransform } from '@/core/grid/helpers/utils'
  import { getControlPosition, createCoreData } from '@/core/grid/helpers/draggableUtils'
  import { getDocumentDir } from '@/core/grid/helpers/DOM'
  ...
  export default {
    ...
    computed: {
      ...
      active () {
        return this.index === this.activeItemIndex
      },
      ...
      calcPosition: function (x, y, w, h) {
        ...
        if (this.renderRtl) {
          ...
          out = {
            ...
            top: y === 0 ? 0 : this.rowHeight * y + 3,
            // top: Math.round(this.rowHeight * y + (y + 1) * this.margin[1]),
            width: w === Infinity ? w : Math.round(colWidth * w + Math.max(0, w - 1) * this.margin[0]),
            height: h === Infinity ? h : this.rowHeight * h
            // height: h === Infinity ? h : Math.round(this.rowHeight * h + Math.max(0, h - 1) * this.margin[1])
          }
        }
        return out
